...
num_cascade = cv2.CascadeClassifier('haarcascade_russian_plate_number.xml')
...
while Count < 50:
    ...
    if np.average(gray) > 110:                                                                      # Testing the brightness of the image
        ...
        for (x, y, w, h) in faces:                                                                  # Frames  LOCATION X, Y  WIDTH, HEIGHT
            ...
            cv2.putText(gray, "Numplate DETECTED",(x+(w//2),y-5), cv2.FONT_HERSHEY_DUPLEX, .4, WHITE)
            #tc1 = img_as_ubyte(tc)
            #print(tc)
            #cv2.putText(tc)           
            if Img is not None:
            ...
            Count = Count + 1
    cv2.imshow('Numplate Recognition System Capture Faces', gray)                                       # Show the video
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break
print('Numplate CAPTURE FOR THE SUBJECT IS COMPLETE')
cap.release()
